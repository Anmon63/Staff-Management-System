...
function StaffDetails() {
  ...
  const getStaff = () => {
    ...
    .then(res=>{
        const filtered = res.data.filter(item => item.role === "teamlead")
        setStaff(filtered)
...
const handleDelete = (id) => {
    ...
    if (confirmDelete) {
        axios.delete(`http://localhost:8000/staff/api/staffdet/${id}/`)
            .then(() => {
                alert("Teamlead member deleted successfully.");
                getStaff(); // Refresh the table
            })
            .catch(err => {
                console.error("Delete error:", err);
                alert("Error deleting teamlead member.");
            });
    }
};
return (
